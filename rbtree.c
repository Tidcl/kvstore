...
int rbtree_set(const char* key, const char* value){
	...
	// printf("search %s \n", key);
	rbtree_node* snode = rbtree_search(&tree, key);
	if(snode != tree.nil){
		return -1;
	}
...
rbtree_node* rbtree_get(const char* key){
	if (!key) return NULL;
...
int rbtree_exist(const char* key){
	return rbtree_search(&tree, key) != tree.nil ? 1 :  0;
}

int rbtree_del(const char* key){
	rbtree_node* node = rbtree_search(&tree, key);
	if(node == tree.nil){
		return -1;
	}
...
#define RBTREE_PERSISTENCE_FILE "./PersistenceRBtreeData"

void rbtree_front(rbtree_t* tree, rbtree_node* node, FILE* filefd){
	if(node != tree->nil){
        size_t len = strlen(node->key);
        fwrite(&len, sizeof(size_t), 1, filefd);
        fwrite(node->key, sizeof(char), len, filefd);
        len = strlen(node->value);
        fwrite(&len, sizeof(size_t), 1, filefd);
        fwrite(node->value, sizeof(char), len, filefd);

		rbtree_front(tree, node->left, filefd);
		rbtree_front(tree, node->right, filefd);
	}
}

int rbtree_persistence(rbtree_t* tree){
    //持久化到文件
    FILE* file = fopen(RBTREE_PERSISTENCE_FILE, "wb");
    if(file == NULL){
        return -1;
    }
...
void rbtree_unpersistence(rbtree_t* tree){
    //持久化文件构造
    FILE* file = fopen(RBTREE_PERSISTENCE_FILE, "rb");
    if(file == NULL){
        return -1;
    }
    ...
    while(feof(file) == 0){
        ...
        if(keybuffer[0] != '\0'){
            rbtree_set(keybuffer, valbuffer);
			free(keybuffer);
			free(valbuffer);
        }
    }
