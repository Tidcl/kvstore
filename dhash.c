...
hash_node *create_hash_node(const char *key, const char * value) {
    hash_node *node = (hash_node *) malloc(sizeof(hash_node));
    // node->key = strdup(key);
    // node->value = value;

    memcpy(node->key, strdup(key), MAX_KEY_LEN);
    memcpy(node->value, value, MAX_VALUE_LEN);
...
const char* search(hash_table *table, const char *key) {
    int index = hash_function(key, table->size);
